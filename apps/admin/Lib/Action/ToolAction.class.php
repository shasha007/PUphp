...
class ToolAction extends AdministratorAction {
    ...
    public function sqlUpdate()
    {
        ...
        foreach($file as $v){
            ...
            $this->_checkDeveloper($v,$data);
        }
        $this->assign('list',$data);
        $this->display();
    }
    private function _checkDeveloper($uname,&$data)
    {
        $version = M('sql_update')->getField('version',"uname='$uname'");
        $version = intval($version);
        $dir = SITE_PATH."/sql/".$uname;
        $file = scandir($dir);
        foreach($file as $v){
            $fileVersion = substr($v, 0, strpos($v, '.'));
            if($fileVersion > $version){
                $data[] = array('uname'=>$uname,'file'=>$v);
            }
        }
        return $res;
    }
    private function _doDeveloperSql($uname){
        $version = M('sql_update')->getField('version',"uname='$uname'");
        $insert = false;
        if(is_null($version)){
            $insert = true;
        }
        $nowVersion = intval($version);
        $dir = SITE_PATH."/sql/".$uname;
        $file = scandir($dir);
        foreach($file as $v){
            $fileVersion = substr($v, 0, strpos($v, '.'));
            if($fileVersion > $nowVersion){
                $res = M('')->executeSqlFile($dir.'/'.$v);
                echo '<strong>'.$uname.' / '.$v.'：</strong>';
                if(!empty($res)){//错误
                    echo '<br /><span class="cRed">';
                    echo $res['error_code'];
                    echo '</span><br />';
                    return false;
                }else{
                    ...
                    if($insert){
                        M('sql_update')->flush();
                        $data['uname'] = $uname;
                        M('sql_update')->add($data);
                        $insert = false;
                    }else{
                        M('sql_update')->where("uname='$uname'")->save($data);
                    }
                    echo ' 导入成功！<br />';
                }
            }
    ...
    //查看sql文本
    public function ajaxSqlFile()
    {
        $path = SITE_PATH."/sql/".$_REQUEST['uname'].'/'.$_REQUEST['file'];
        $this->assign('file',  file_get_contents($path));
        $this->display();
    }
    //执行全部sql
    public function doSql()
    {
        ...
        foreach($file as $v){
            ...
            $this->_doDeveloperSql($v);
        }
    }
    public function upSqlCache()
    {
        $res = M('')->query("select table_name from information_schema.tables where table_schema='2012xyhui';");
        foreach ($res as $v) {
            $key = '_fields/'.$v['table_name'];
            if(Mmc($key) !== false){
                echo $v['table_name'].', ';
                Mmc($key,null);
            }
