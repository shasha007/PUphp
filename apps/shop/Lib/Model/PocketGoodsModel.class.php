...
class PocketGoodsModel extends Model {
    ...
    public function addProduct($data) {
        ...
        $pData['cid'] = $data['cid'];
        $pData['num'] = $data['num'];
        $pData['lowestShoufu'] = $data['lowestShoufu'];
        $pData['market'] = $data['market'];
        $pData['lowest'] = $data['lowest'];
        $pData['stock'] = $data['stock'];
        $pData['profitId'] = $data['profitId'];
        $gid = $this->add($pData);
        if ($gid) {
            $this->setField('ordernum', $gid, 'id='.$gid);
            $oData['content'] = $data['content'];
            $oData['imgs'] = $data['imgs'];
            $oData['color'] = $data['color'];
            $oData['gid'] = $gid;
            $oData['desc'] = $data['desc'];
            M('pocket_goods_opt')->add($oData);
            return $gid;
        }
        $this->error = '添加失败';
        return 0;
    ...
    public function apiGoods(){
        $map['isDel'] = 0;
        $map['isHot'] = 1;
        $list = $this->where($map)->field('id,pic,name,price')->limit(3)->findAll();
        foreach($list as &$v){
            $v['pic'] = tsMakeThumbUp($v['pic'],200,200);
            $v['url'] = 'http://pocketuni.net/index.php?app=shop&mod=PocketShop&act=detail&id='.$v['id'];
            unset($v['id']);
        }
        return $list;
    }
    ...
    public function updateProduct($data) {
        ...
        $pData['lowestShoufu'] = $data['lowestShoufu'];
        $pData['profitId'] = $data['profitId'];
        $gid = $this->where('id='.$data['id'])->save($pData);
        $oData['content'] = $data['content'];
        $oData['desc'] = $data['desc'];
        $oData['imgs'] = $data['imgs'];
        $oData['color'] = $data['color'];
        M('pocket_goods_opt')->where('gid=' . $data['id'])->save($oData);
        return $data['id'];
    }
...
//        return $this->setField('isDel', 1, 'id=' . $id);
    ...
    {
        $list = $this->where('isDel=0 and isHot=1')->field('id,name,price,pic,stock')->findAll();
        return $list;
    }
