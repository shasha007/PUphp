...
class LendModel extends CMBModel
{
	 ...
	 */
	public function DCPAYMNT($finance_id,$uid,$sum,$studentCardNo,$studentCardAccount)
	{
		...
		if($checkTel['code'] == '1') {
			...
			if(in_array($this->revData['INFO']['RETCOD'], array('0','-1','-9')) && isset($this->revData['NTQPAYRQZ'])) {
				...
				if($this->revData['NTQPAYRQZ']['REQSTS'] == 'FIN' && $this->revData['NTQPAYRQZ']['RTNFLG'] == 'F') {
					M('bank_finance')->where(array('id'=>$finance_id))->save(array('status'=>40));
	...
	// 					$res = D('GetPaymentInfo','bank')->GetPaymentInfo(10006,$sdate,$edate);
			...
			M('bank_finance')->where(array('id'=>$finance_id))->save(array('status'=>40));
			return array('code'=>'-1','msg'=>'check tel failed');
		}
	}
	/**
...
// 		if($this->revData['INFO']['RETCOD'] != '0') return array('code'=>0,'msg'=>'代发失败','result'=>$this->revInfo);
	...
	public function saveOrder($finance_id='',$uid,$bank_order_id='',$bank_code=0,$bank_msg='')
	{
		...
		if($this->order_id) {
                    if($bank_code==1){
                        $res1 = M($this->tableName)->where(array('order_id'=>$this->order_id))->find();
                        $finance = M('bank_finance')->where('id='.$res1['finance_id'])->find();
                        doQuery("UPDATE ts_bank_finance SET status=1 WHERE id='$res1[finance_id]'");
                        doQuery("UPDATE ts_bank_finance_detail  SET status=1 WHERE bank_finance_id='$res1[finance_id]'");
                        doQuery("UPDATE ts_bank_card SET surplus_line=surplus_line-{$finance[money]} WHERE id='{$finance[bank_card_id]}'");
                    }
			$data = array(
						'etime'=>$datetime,
