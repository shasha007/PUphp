...
class WeiboModel extends Model{
	 ...
	 */
     function publish($uid, $data, $from=0, $type=0, $type_data, $sync, $from_data){
        $data['content'] = getShort($data['content'],150);
     	$data['content'] =t( $data['content'] );
     	if($id = $this->doSaveWeibo($uid, $data, $from , $type ,$type_data, $from_data)){
            // 话题里微博数量+1
            if (isset($data['themes_id']) && $data['themes_id']>0) {
                M('weibo_themes')->where('id='.$data['themes_id'])->setInc('weibo_num') ;
            }
    ...
    private function _isEmptyContent($content){
    ...
    function doSaveWeibo($uid, $data, $from=0, $type=0, $type_data=null, $from_data=null){
        if($this->_isEmptyContent($data['content'])){
            return false;
        }
        ...
        $save['uid']			= $uid;
        $gzh = model('UserGz')->getGzUids($uid);
        if(in_array($uid, $gzh)){
            $sid = 5;
        }else{
		...
		if(!empty($data['themes_id']))
		{
			$save['themes_id']            = $data['themes_id'];
		}
		if(!empty($data['isHide']))
		{
			$save['isHide']            = $data['isHide'];
		}
        $save['sid']			= $sid;
        $save['transpond_id']	= isset($data['transpond_id']) ? intval( $data['transpond_id'] ) : 0;
        ...
        if($type){
			...
			$save['type'] = isset($data['type']) ? intval( $data['type'] ) : 0;
    ...
    public function getOneApi($id, $value, $uid = 0) {
        ...
        if(!$id)
            $id = $value['weibo_id'];
        $value['ate_uid'] = 0;
        if(preg_match("/@([\w\x{4e00}-\x{9fa5}\-]+)/u",$value['content'],$matchs)) {
            $ateUser = M('user')->getField('uid', "uname='".$matchs[1]."'");
            $value['ate_uid'] = intval($ateUser);
        }
        $value['uname'] = getUserName($value['uid']);
        $value['face'] = getUserFace($value['uid'],'b');
        if ($value['type'] == 1 && $value['transpond_id'] == 0) {
            $value['type_data'] = unserialize($value['type_data']);
            $value['type_data']['picurl'] = PIC_URL.'/data/uploads/' . $value['type_data']['picurl'];
            $value['type_data']['thumbmiddleurl'] = PIC_URL.'/data/uploads/' . $value['type_data']['thumbmiddleurl'];
            $value['type_data']['thumburl'] = PIC_URL.'/data/uploads/' . $value['type_data']['thumburl'];
        }
        ...
        $value['content'] = htmlspecialchars_decode(keyWordFilter($value['content']), ENT_QUOTES);
    ...
    public function __parseTemplate( $value ){
    	...
    	$typedata = !empty($value['type_data']) ? unserialize( $value['type_data'] ) : '';
	...
	//获取我的话题列表
	function getMyWeibo($uid,$count,$page){
		$offset = ($page - 1) * $count;
		$map['uid']=$uid;
		$map['isdel']=0;
		$weibo = M('weibo')	->where($map)
							->field('uid,type,cTime,weibo_id,type_data,content')
							->order('weibo_id desc')
							->limit("$offset,$count")
							->select();
		foreach($weibo as &$v)
		{
			//用户头像
			$v['user_face'] = getUserFace($v['uid']) ;
			//用户姓名
			$v['uname'] = getUserName($v['uid']) ;
			//用户性别：1为男2为女
			$v['user_sex'] =getUserSex($v['uid']);
			//用户学校
			$v['school']=getUserInfo($v['uid'], '', '');
			$v['school']=$v['school']['school'];
			...
			$v['time']=dateFormat($v['cTime']);
			$v['type_data']=unserialize($v['type_data']);
            $v['content']=replaceSelfEmoji($v['content']);
			//dump($v['type_data']) ; 
            if(!empty($v['type_data'][0]))
			{
				foreach ($v['type_data'] as &$item)
				{
					$item['picurl'] = PIC_URL.'/data/uploads/' . $item['picurl'];
					$item['thumbmiddleurl'] = PIC_URL.'/data/uploads/' .$item['thumbmiddleurl'];
					$item['thumburl'] = PIC_URL.'/data/uploads/' . $item['thumburl'];
				}
			}
			elseif($v['type_data'])
			{
					$pi['picurl'] = PIC_URL.'/data/uploads/' .$v['type_data']['picurl'];
					$pi['thumbmiddleurl'] = PIC_URL.'/data/uploads/' .$v['type_data']['thumbmiddleurl'];
					$pi['thumburl'] = PIC_URL.'/data/uploads/' . $v['type_data']['thumburl'];
					unset($v['type_data']) ;
					$v['type_data'][] = $pi ;			
			}else
				{
    ...
    // 新的微博删除
    public function doDelWeibo($weibo_id,$uid){
        if($info = $this->where("weibo_id={$weibo_id} AND uid={$uid} ")->find()){
            if($info['isdel'] == 0 && $this->setField('isdel', 1, "weibo_id={$info['weibo_id']} AND isdel=0")){
                D('WeiboThemes','weibo')->decrease($weibo_id) ;
                // 不知道是否要加 减积分操作 先不管
                return true;
            }
        }
