...
//        if($key=='#妈妈再也不用担心我丢东西了#'){
//            $listMap['sid'] = getUserField($uid, 'sid');
//        }
//        $listMap['content'] = array('like',$key.'%');
//        $listMap['isdel'] = 0;
//        $offset = ($page - 1) * $limit;
...
//        $weibo_id_list = getSubByKey($data, 'weibo_id');
        ...
        $offset = ($page - 1) * $limit;
        $topicId = D('Topic', 'weibo')->getTopicId($key);
        $listMap['a.topic_id'] = $topicId;
        if($key=='#失物招领#'){
            $sid = getUserField($uid, 'sid');
            $listMap['b.sid'] = array('in',array($sid,5));
        }
        $listMap['b.isdel'] = 0;
        $data = $this->table("ts_weibo_topic_link AS a ")
            ->join("ts_weibo AS b ON  a.weibo_id=b.weibo_id")
            ->field('b.weibo_id')->where($listMap)
            ->order('weibo_id DESC')->limit("$offset,$limit")->findAll();
        $weibo_id_list = getSubByKey($data, 'weibo_id');
        ...
        foreach($list as $k=>$v){
            ...
            $v['content'] = preg_replace('/^#[^#]+#/','',$v['content'],1);
...
//	}
    ...
    function getIndex($num=10){
    	$list = $this->where("transpond_id=0 AND type=0 AND isdel=0")->limit($num)->order('weibo_id DESC')->findall();
    	return $list;
    }
...
//    				 ->findPage(10, $count);
    ...
    //提到我的 Api版
    public function getAtmeApi($uid,$limit = 10, $page = 1){
        $offset = ($page - 1) * $limit;
	//查询进行优化
	$list = M()->Table("`{$this->tablePrefix}weibo` AS w LEFT JOIN `{$this->tablePrefix}weibo_atme` AS a ON w.weibo_id = a.weibo_id LEFT JOIN `{$this->tablePrefix}user_blacklist` AS b ON w.uid = b.fid AND b.uid = {$uid}")
				   ->field('w.*')
    ...
    private function _paramResultData($list,$uid,$page=false){
        ...
        */
    	...
    	//jun  获取当前用户所关注用户的最新微博
	public function friends_weibo($uid,$limit = 10)
	{
