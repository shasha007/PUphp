...
class FollowModel extends Model {
    ...
    public function getNowFollowing($uid) {
        ...
        if ($list) {
            $res = getSubByKey($list, 'fid');
        }
        //公众号
        $gzh = model('UserGz')->getGzUids($uid);
        $res = array_merge($res, $gzh);
        return $res;
    }
     ...
     */
    public function dofollow($mid, $fid, $type = 0,$froms=0) {
    ...
    public function unfollow($mid, $fid, $type = 0) {
        ...
        //公众号
        if($type==0){
            $unfollow = model('UserGz')->unfollow($mid,$fid);
            if($unfollow){
                $deleted = true;
            }
        }
    ...
    function getList($uid, $operate, $type = 0, $gid = NULL, $limit = 10) {
        ...
        if ($operate == 'following') { //关注
            if (is_numeric($gid) && $type == 0) {
                $list = $this->where('uid='.$uid.' and type='.$type.' and group_id='.$gid)->findPage($limit);
            } else {
                $total = $this->where("uid=$uid AND type=$type")->count();
                $gzh = model('UserGz')->getGzUids($uid);
                $gzhCount = count($gzh);
                $list = $this->where("uid=$uid AND type=$type")->order('ctime DESC')->findpage($limit,$total+$gzhCount);
//公众号
if(empty($list['data'])){
    $listCnt = 0;
}else{
    $listCnt = count($list['data']);
}
$page = $list['nowPage'];
if($listCnt<$limit){
    $gzPage = 1;
    ...
    if($listCnt==0){
        if($total>0){
            ...
            $bu = $totalPage*$limit-$total;
            if($bu>0){
                $gzPage += 1;
            }
        }
    }
    $uids = getSubByKey($list['data'], 'fid');
    $adduids = array_diff($gzh,$uids);
    $rest = $limit-$listCnt;
    $i = 0;
    if($gzPage==1){
        $start = 0;
    }else{
        $start = $limit*($gzPage-2)+$bu;
    }
    $key = $start+$i;
    while($i<$rest && isset($adduids[$key])) {
        $value = $adduids[$key];
        if($uid!=$value){
            $list['data'][] = array('id'=>$uid.'_'.$value,'uid'=>$uid,'fid'=>$value,'type'=>0,'gzh'=>1);
            $i++;
        }
        $key += 1;
    }
            ...
            $list = $this->where("fid=$uid AND type=$type")->order('ctime DESC')->findpage($limit);
...
//    		$user_count_model->setUserFollowerCount($uids);
    ...
    // 修改备注
    public function changeRemark($uid,$fid,$remark){
        $map['uid'] = $uid ;
        $map['fid'] = $fid ;
        $data['remark'] = $remark ;
    ...
    // 获取备注
    public function getRemark($uid,$fid){
        $map['uid'] = $uid ;
        $map['fid'] = $fid ;
        return $this->where($map)->getField('remark') ;
    }
}
