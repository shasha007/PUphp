 ...
 * 活动显示于学校
 */
class FundEventSchoolModel extends Model {

    //计算投放学校
    public function addSchool($fundEvnetId) {
        $data['fundEvnetId'] = $fundEvnetId;
        $data['china'] = 1;
        $this->where("fundEvnetId=$fundEvnetId")->delete();
        if(!isset($_POST['prov'])){
            $this->add($data);
            return true;
        }
        ...
        $this->_insertSchool($fundEvnetId,$allProv,$allCity,$sids);
        return true;
    }
    //前台编辑地区
    public function editbarSchool($fundEvnetId){
        $list = $this->where("fundEvnetId=$fundEvnetId")->findAll();
        $res = array();
        foreach ($list as $v) {
            if($v['china']==1){
                return array();
...
//        var_dump($res);die;
    ...
    //是否该校可申请任务
    public function isSchoolRw($sid,$fundEvnetId){
        $map['fundEvnetId'] = $fundEvnetId;
    ...
    private function _insertSchool($fundEvnetId,$allProv,$allCity,$sids){
        $data = array('fundEvnetId' => $fundEvnetId);
