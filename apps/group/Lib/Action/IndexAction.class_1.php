...
class IndexAction extends BaseAction {
    ...
    public function _initialize() {
        ...
        //jun 有社团的学校
        $tschools = M('group')->field('school')->findAll();
        $str = array();
        foreach ($tschools as $k => $v) {
            $str[$k] = $v['school'];
        }
        $str = array_unique($str);
        $str = implode(',', $str);
        $schools = M('school')->where("id  IN($str)")->field('title as t,id as a')->findAll();
        $school = intval($_GET['school']);

        if ($school == 0) {
            $school = intval($_SESSION['school']);
        } else {
    ...
    public function getSchool() {
        if (isset($this->schools[$this->school]['a'])) {
            return $this->schools[$this->school]['a'];
        }
        return 0;
    }
...
//        var_dump($countHash);die;
    ...
    public function doAdd() {
        ...
        if (trim($_POST['dosubmit'])) {
            ...
            $group['school'] = intval($this->user['sid']);
    ...
    //最新话题
    function newtopic() {
        $this->assign('mymanagegroup', $this->group->mymanagegroup($this->mid));  //我管理的社团
        $this->assign('myjoingroup', $this->group->myjoingroup($this->mid));    //我加入的社团

        $this->assign('newTopic', $this->group->getnewtopic($this->mid));         //最新话题 自己加入的社团和自己创建的
    ...
    function issue() {
        ...
        if ($myAllGroup) {
            ...
            $this->display();
        } else {  //如果没有社团，则跳转到创建页面
            $url = __APP__ . "/Index/add";
            $this->assign('jumpUrl', $url);
            $this->error('你还没有创建社团，请你先创建社团！');
        }
    }
...
//            print_r($map);die;
        ...
        $countList = D('Group', 'group')->field('count(1) as count,cid0')->where("cid0 IN (" . implode(',', $cidArr) . ") AND is_del=0 AND school=$school")->group('cid0')->findAll();
        foreach ($countList as $v) {
            isset($countHash[$v['cid0']]) && $countHash[$v['cid0']] = $v['count'];
        }
        $sum=0;
