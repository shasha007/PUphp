...
class GroupAction extends BaseAction {
    ...
    public function search() {
        ...
        $this->assign($data);
        $this->setTitle('搜社团: ' . $data['search_key']);
        $this->display('index');
    }
    ...
    public function joinGroup() {
        if (isset($_POST['addsubmit'])) {
            ...
            if ($this->is_invited) {
                ...
                $message_data['title'] = "申请加入社团 {$this->groupinfo['name']}";
                $message_data['content'] = "你好，请求你批准加入“{$this->groupinfo['name']}” 社团，"
                        . '<a href="' . U('group/Manage/membermanage', array('gid' => $this->gid, 'type' => 'apply')) . '"> 【点此】 </a>' . '进行操作。';
    ...
    public function moveGroup() {
        ...
        if ($_POST['sendsubmit']) {
            $toUserId = t($_POST["fri_ids"]);
            if (empty($toUserId)) {
                $this->error('请选择转移对象！');
                exit();
            }
            $user = M('user');
            $name = $user->where("uid=$toUserId")->field('uname')->find();
            $resultA = $group->where("id=$this->gid")->setField('uid', $toUserId);
            $resultB = $member->where("gid=$this->gid")->setField(array('uid', 'name'), array($toUserId, $name['uname']));
            if ($resultA && $resultB) {
    ...
    public function disband() {
        ...
        if (!iscreater($this->mid, $this->gid))
            exit('你没有权限');
        $disband = D('Group')->where('id=' . $this->gid)->field('disband')->find();
        if ($disband['disband'] == 1)
            exit('你已经申请过了，不能重复申请');
        D('Group')->where('id=' . $this->gid)->setField('disband', 1);
        exit('1');
    }
