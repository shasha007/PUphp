...
class BaseAction extends Action {
    ...
    protected function _initialize() {
        $this->assign('all_my_app', D('Group')->getAllMyGroupMin($this->mid, 0));
        ...
        $groupinfo = D('Group')->where('id=' . $this->gid . " AND is_del=0 AND school=0")->find();
        /* + ------ 后台全局限制 START */
    ...
    protected function base() {
        ...
        $this->assign('all_my_app', D('Group')->getAllMyGroupMin($this->mid));
    }
    ...
    private function _rights(){
        if($this->mid){
            $map['uid'] = $this->mid;
            $map['user_group_id'] = 1;
            $this->rights['addGroup'] = false;
            if(M('user_group_link')->where($map)->field('user_group_id')->find()){
                $this->rights['addGroup'] = true;
            }
        }
