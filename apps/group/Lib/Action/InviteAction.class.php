...
class InviteAction extends BaseAction {
	...
	public function create()
	{
		...
		if($_POST['sendsubmit']) {
			...
			$message_data['title']   = "邀您加入社团 {$this->groupinfo['name']}";
			$message_data['content'] = "你好，诚邀您加入“{$this->groupinfo['name']}” 社团，点击" . U('group/Group/index', array('gid'=>$this->gid)) . '进入。';
            ...
            if (count($toUserIds) > 0) {
            	...
            	$this->error('邀请失败,您的好友可能已经加入了该社团！');
            	//echo -1;
            }
			exit;
		}
