...
class ManageAction extends BaseAction{
	...
	public function do_create_album() {
		...
		if(intval($_POST['privacy'])==4)
			$album->privacy_data	=	md5($_POST['privacy_data'].'_'.$this->mid);
	...
	public function do_update_album() {
		...
		/*   处理相册信息  */
			...
			//如果相册隐私数据发生变化
                        $album_privacy_data = md5($album_privacy_data.'_'.$albumInfo['userId']);
	...
	public function do_update_photo() {
		$result = D('Photo', 'photo')->apiUpdatePhoto($this->mid);
		//返回
		if($result){
			echo 1;//成功
		}else{
