...
class AttachAction extends Action{
	...
	public function ewebeditor(){
		...
		if(is_array($info['info'])){
			$image_url	=	PIC_URL.'/data/uploads/'.$info['info'][0]['savepath'].$info['info'][0]['savename'];
		}
	...
	public function kind(){
		...
		//上传成功
		if($info['status']==true){
                    echo json_encode(array('error' => 0, 'url' => $image_url));
		}else{
                    echo json_encode(array('error' => 1, 'message' => $info['info']));
		}
                exit;
	}
	...
	public function thumbImage() {
		...
		$face_url	=	PIC_URL.'/data/userface/'.$this->mid;
