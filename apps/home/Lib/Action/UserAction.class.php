...
//        Session::pause();
        ...
        $type = !empty($_GET['weibo_type']) ? h($_GET ['weibo_type']) : '';
...
//            $data = $this->__getWeiboList($install_app,$type); //显示微博列表
        $follow = D('Operate', 'weibo')->friends_weibo($this->mid, 5); //我关注的人的微博
...
//        $this->assign ( $data );
        ...
        //活动
       $events =  D('Event','event')->getHoneEvent($this->user['sid']);
            $this->assign('events', $events);
        $this->setTitle(L('my_index'));
        $map['isDel'] = 0;
        $notice = M('notice')->where($map)->field('id,title,front,cTime')->order('id DESC')->limit(8)->findAll();
...
//广告统计
    public function adRecord() {
        $adid = intval($_REQUEST['adid']);
        if ($adid < 0)
            return false;
        $res = D('Ad', 'admin')->setInc('count', 'id=' . $adid);
        if ($res) {
            $data['uid'] = $this->mid;
            ...
            $data['ctime'] = time();
            $data['from'] = 3;
            M('ad_record')->add($data);

        }
         $type = D('Ad', 'admin')->getField('type','id='.$adid);
         if($type ==1){
