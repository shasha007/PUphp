...
class IndexAction extends Action{
	...
	public function _initialize() {
        ...
        public function qswj(){
            $domain = getDomainLink($this->user['schoolEvent']['domain']);
            $id=intval($_REQUEST['id']);
            if($id){
                $ref = $domain.'/index.php?app=event&mod=Survey&act=detail&id='.$id;
            }else{
                $ref = $domain.'/index.php?app=event&mod=Survey&act=index';
            }
            redirect($ref);
            die;
        }
	...
	public function testApi(){
	    $type = $_GET['type'];
	    $cname = $_GET['cname'];   //获取接口名
	    $mname = $_GET['mname'];   //获取方法名
	    $mid = $_SESSION['mid'];
	    $api = array();               //将api配置成一个三维数组，分别为api类名，该类名的方法名，该方法所需的参数
	    $map = array();
	    $map['uid'] = $mid;
	    $map['type'] = 'location';
	    $user = D('Login');
	    $userList = $user->where($map)->field('oauth_token,oauth_token_secret')->select();
	    $api = array(
	        'Album' => array(
	            'albumList' => array(
	                'uid' => $mid,
	    ...
	    if($type == 'getmethod'){
	        $this->ajaxReturn($api[$cname]);
	        die;
	    }
	    if($type == 'getparams'){
	        $this->ajaxReturn($api[$cname][$mname]);
	        die;
	    }
	    $this->assign(array(
	        'api' => $api,
	        'uid' => $mid,
	        'oauth_token' => $userList[0]['oauth_token'],
	        'oauth_token_secret' => $userList[0]['oauth_token_secret']
	    ));
	    $this->display();
	}
