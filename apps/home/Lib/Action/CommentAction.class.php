    ...
    class CommentAction extends Action{
        ...
        public function getComment(){
        	...
        	$map['type']  = t($_REQUEST['type']);
         	$list = model('GlobalComment')->where($map)->order('top DESC,id DESC')->findpage(10);
        ...
        public function doAddComment() {
        	...
        	if ( $_POST['to_id'] > 0 )
        		$former_comment = M('comment')->where("`id`='{$_POST['to_id']}'")->field('uid,comment')->find();
...
*/
		...
		public function doTop()
		{
			if ( empty($_POST['id']) )
				return ;
			if(model('GlobalComment')->topComment($_POST['id'],$_POST['type'])){
				//积分处理
				echo 1;
			}else{
				echo 0;
