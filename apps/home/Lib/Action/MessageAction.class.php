...
class MessageAction extends Action
{
	...
	public function doPost() {
		...
		// 发送私信
		$rongyun_group['fromUserId']    = $this->mid;
		$rongyun_group['toUserId']   = $_POST['to'];
		$rongyun_group['content']   = strip_tags($_POST['content']);
		$rongyun_data['do_action']  = json_encode(array('Rongyun','messagePublish'));
	...
	public function doReply() {
		...
		$res = model('Message')->replyMessage( intval($_POST['id']), $_POST['reply_content'], $this->mid );
		if ($res) {
			echo 1;
		}else {
			echo 0;
