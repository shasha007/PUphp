...
class SquareAction extends Action
{
    ...
    public function index(){
    	...
    	if(empty($data['userlist'])) {
	    	$data['userlist'] = M('weibo')->where("transpond_id=0 AND isdel=0 AND content LIKE '%".$data['aboutkey']."%'")->limit(10)->order('weibo_id DESC')->findAll();
	...
	public function index_weibo(){
    	...
    	switch ($data['type']) {
    		...
    		case 'friend'://好友动态
                        ...
                        $following = $daoFollow->getNowFollowing($this->mid);
                        $following[] = $this->mid;
                        $uids = implode($following, ',');
                        $map = " `uid` IN ($uids)";
    			$order = 'weibo_id DESC';
    			break;
    		case 'person'://个人动态
        ...
        if($data['type'] == 'friend'||$data['type'] == 'person'){
            $data['type'] = 'index';
        }
    	if($data['list']){
	    	$this->assign($data);
