...
class SpaceAction extends Action {
    ...
    function _initialize() {
        if(!isset($_GET['uid'])){
        ...
        }elseif (!is_numeric($_GET['uid']) && is_string($_GET['uid'])) {
            ...
            $user = D('User')->getUserByIdentifier($this->uid);
            if (!$user) {
    ...
    public function index() {
        ...
        if ($this->uid != $this->mid || $_GET['person']) {
            $type = 'person';
        } else {
            $type = 'friend';
        }
        $this->assign('fro',$type);
        $this->assign($data);
        $this->display();
    }
    ...
    public function profile() {
        $pUserProfile = D('UserProfile','hone');
        $pUserProfile->uid = $this->uid;
        $data['userInfo'] = $pUserProfile->getUserInfo(false);
        // 个人情况-钩子
    ...
    //编辑个性签名
      public function editSign() {
        ...
        if (mb_strlen($val, 'utf8') > 50) {
            $this->error('个性签名字数不能超过50个字');
        }
        if ($this->mid == $this->uid) {
              $dao = D('User', 'home');
        $res = $dao->upField($this->mid,'sign',$val);
        } else {
            $this->error('非法操作');
        }
        if ($res) {
